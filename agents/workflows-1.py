...
def analisar_detalhes_evento(descricao: str) -> DetalhesEvento:
    ...
    response = client.responses.parse(
        model=modelo,
        input=f"{contexto_data} Extraia informações detalhadas do evento. Quando as datas fizerem referência a 'próxima terça-feira' ou datas relativas similares, use a data atual com referência.",
